...
import { Trophy, MessageCircle, AlertCircle, RefreshCw } from 'lucide-react';
import { useGetCurrentGiveaway, getGetCurrentGiveawayQueryKey, GiveawayParticipant } from '@workspace/api-client-react';

export default function LiveView() {
  const { data: giveaway, isLoading, error } = useGetCurrentGiveaway({
    query: {
  ...
  if (isLoading && !giveaway) {
    return (
      <div className="min-h-[100dvh] bg-background text-foreground flex items-center justify-center dark">
        <div className="flex flex-col items-center gap-4">
          <RefreshCw className="w-10 h-10 animate-spin text-primary opacity-70" />
          <p className="font-bold text-lg text-primary/80 animate-pulse">خەریکی بارکردن...</p>
        </div>
      </div>
    );
  }

  if (error || (!isLoading && (!giveaway || !giveaway.exists))) {
    return (
      <div dir="rtl" className="min-h-[100dvh] bg-background text-foreground flex items-center justify-center p-6 dark">
        <div className="max-w-md w-full bg-card/40 backdrop-blur-xl border border-white/10 rounded-3xl p-8 text-center shadow-2xl flex flex-col items-center gap-4">
          <AlertCircle className="w-16 h-16 text-destructive opacity-80" />
          <h2 className="text-2xl font-bold text-white">هیچ خەڵاتێک نەدۆزرایەوە</h2>
          <p className="text-muted-foreground">لە ئێستادا هیچ خەڵاتێکی ڕاستەوخۆ بوونی نییە یان هەڵەیەک ڕوویدا لە وەرگرتنی زانیارییەکان.</p>
        </div>
      </div>
    );
  }
  ...
  const rest = participants.slice(3, 50);
  const currentStatus = giveaway?.status || 'idle';
  const totalComments = giveaway?.totalComments || 0;
  const prizeTitle = giveaway?.prizeTitle || 'خەڵاتی گیادەرمانی';
  const prizeCount = giveaway?.prizeCount || 3;
  ...
  return (
    <div dir="rtl" className="dark min-h-[100dvh] bg-background text-foreground overflow-x-hidden overflow-y-auto font-sans selection:bg-primary/30">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mt-2">
                ...
                <span className="text-muted-foreground font-mono font-medium flex items-center gap-2 text-sm sm:text-base">
                  ...
                  <span dir="ltr">{totalComments.toLocaleString()}</span> کۆی کۆمێنتەکان
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col lg:flex-row gap-8 lg:gap-12 w-full lg:items-end pb-4 lg:pb-8 min-h-0">
          ...
          {participants.length === 0 ? (
            <div className="flex-1 flex items-center justify-center w-full min-h-[400px]">
              <div className="text-center space-y-4 p-8 bg-card/20 rounded-3xl border border-white/5 backdrop-blur-sm">
                <MessageCircle className="w-16 h-16 mx-auto text-muted-foreground/50" />
                <h3 className="text-xl font-bold text-white/80">هیچ بەشداربوویەک نییە</h3>
                <p className="text-muted-foreground">چاوەڕێی هاتنی کۆمێنتەکانین...</p>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="w-full lg:flex-1 flex items-end justify-center gap-1 sm:gap-4 lg:gap-10 min-h-[300px] md:min-h-[400px] lg:min-h-0 lg:h-full shrink-0">
                <PodiumPosition participant={top3[1]} rank={2} heightClass="h-[100px] sm:h-[180px] lg:h-[40%]" color="from-slate-400 to-slate-600" isCompleted={currentStatus === 'completed'} />
                <PodiumPosition participant={top3[0]} rank={1} heightClass="h-[140px] sm:h-[250px] lg:h-[55%]" color="from-yellow-300 to-yellow-600" isCompleted={currentStatus === 'completed'} />
                <PodiumPosition participant={top3[2]} rank={3} heightClass="h-[70px] sm:h-[130px] lg:h-[30%]" color="from-amber-600 to-amber-800" isCompleted={currentStatus === 'completed'} />
              </div>
                    ...
                    <ul className="space-y-3 sm:space-y-4">
                      <AnimatePresence mode="popLayout">
                        {rest.map((p) => (
                          ...
                          >
                            {currentStatus === 'completed' && p.rank <= prizeCount && (
                            ...
                            <Avatar name={p.displayName} profilePhotoUrl={p.profilePictureUrl || undefined} className="w-10 h-10 sm:w-12 sm:h-12 shadow-md border border-white/10 shrink-0" />
                            <div className="flex-1 min-w-0">
                              <div className="font-extrabold text-base sm:text-lg whitespace-normal break-words leading-tight text-white/95">{p.displayName}</div>
                            </div>
      ...
      {currentStatus === 'completed' && <Confetti />}
    </div>
  );
}
    ...
    <div className="flex flex-col items-center w-[32%] lg:w-[28%] max-w-[280px] relative shrink-0">
      ...
      >
        <div className="relative shrink-0">
          <Avatar name={participant.displayName} profilePhotoUrl={participant.profilePictureUrl || undefined} className={`w-14 h-14 sm:w-24 sm:h-24 lg:w-36 lg:h-36 text-xl sm:text-3xl lg:text-5xl shadow-[0_10px_40px_rgba(0,0,0,0.5)] border-[3px] sm:border-[4px] lg:border-[6px] ${rank === 1 ? 'border-yellow-400' : rank === 2 ? 'border-slate-300' : 'border-amber-600'}`} />
          {isCompleted && (
            <motion.div
        ...
        <div className="mt-3 sm:mt-5 text-center flex flex-col items-center w-full px-1">
          <div className="font-black text-sm sm:text-xl lg:text-3xl whitespace-normal break-words text-center w-full leading-tight drop-shadow-md">{participant.displayName}</div>
