...
5. For a Facebook Reel URL, the admin-only **Select Reel as post** control verifies that Meta identifies the Reel's creator as the configured Page and adds it to the Post selector. This selection does not change the saved giveaway. To replace an existing post, confirm **Change post and reset**, then press **Save**; saving the new post clears its previous comments and ranking. For an empty giveaway, press **Save** after selecting the Reel. The selected Reel remains visible after a page reload even if it is not among the Page's 25 recent posts.
6. **Check Reel comments** separately inspects up to 100 comments without changing the giveaway. Its author counts and sample names describe only the returned page of results. This diagnostic is not a substitute for the end-to-end recording of a real Reel comment in the app's giveaway view; Meta may withhold author identity until the separate profile-access feature is approved.
...
2. Return to the English dashboard. Pause on the Page selector showing the real Page name, then select the real post or verify its Reel URL with **Select Reel as post**. Explain **Change post and reset** when replacing a saved giveaway, **Save**, and **Continue importing comments** with captions or callouts.
