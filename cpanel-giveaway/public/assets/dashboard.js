(() => {
  ...
  const loadPosts = async (assetId, selected='', selectedMessage='') => { el.post.replaceChildren(new Option(t('-- هەڵبژێرە --','-- Select a post --'),''));if(selected){el.post.add(new Option(selectedMessage||t(`هەڵبژێردراو: ${selected}`,`Selected post: ${selected}`),selected));el.post.value=selected}if(!assetId)return;try{const posts=await request('/api/meta/assets/'+encodeURIComponent(assetId)+'/posts');el.post.replaceChildren(new Option(t('-- هەڵبژێرە --','-- Select a post --'),''));posts.forEach(p=>el.post.add(new Option(p.message?p.message.slice(0,60):t(`پۆستی ${new Date(p.createdAt).toLocaleDateString()}`,`Post from ${new Date(p.createdAt).toLocaleDateString()}`),p.id)));if(selected&&!posts.some(p=>p.id===selected))el.post.add(new Option(selectedMessage||t(`هەڵبژێردراو: ${selected}`,`Selected post: ${selected}`),selected));el.post.value=selected}catch(e){message(e.message,true)} };
  const load = async () => { const status=await request('/api/meta/status');csrf=status.csrfToken||csrf;el.callback.textContent=status.callbackUrl||'';if(!status.configured||!status.connected){el.setup.classList.remove('hidden');el.app.classList.add('hidden');return}el.setup.classList.add('hidden');el.app.classList.remove('hidden');el.admin.textContent=status.adminName?t(`بەڕێوەبەر: ${status.adminName}`,`Admin: ${status.adminName}`):'';if(['expired','reconnect_required'].includes(status.tokenStatus))message(t('پەیوەندی Meta پێویستی بە نوێکردنەوەیە.','The Meta connection needs renewal.'),true);assets=await request('/api/meta/assets');const selected=el.asset.value;el.asset.replaceChildren(new Option(t('-- هەڵبژێرە --','-- Select a Page --'),''));assets.forEach(a=>el.asset.add(new Option(`${a.name} (${a.platform==='facebook'?t('فەیسبووک','Facebook'):t('ئینستاگرام','Instagram')})`,a.id)));const current=await request('/api/giveaways/current');renderGiveaway(current);const assetId=current.exists&&!changing?current.assetId:selected;el.asset.value=assetId||'';renderContext();if(assetId)await loadPosts(assetId,current.exists&&!changing?current.postId:'',current.exists&&!changing?current.postMessage:'');await loadRecent(); };
  el.asset.addEventListener('change',()=>{renderContext();loadPosts(el.asset.value)});
  el.save.addEventListener('click',async()=>{try{clearMessage();const data=await request('/api/giveaways/current',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({assetId:el.asset.value,postId:el.post.value,prizeTitle:el.title.value,prizeCount:Number(el.count.value),includeReplies:el.includeReplies.value==='1',reset:changing})});changing=false;pendingReelId='';renderGiveaway(data);await loadPosts(data.assetId,data.postId,data.postMessage);await loadRecent();message(t('پاشەکەوت کرا.','Saved.'));}catch(e){message(e.message,true)}});
  el.change.addEventListener('click',()=>{if(confirm(t('ئەگەر پۆستی نوێ پاشەکەوت بکەیت ڕیزبەندیی پێشوو پاک دەبێتەوە. دڵنیایت؟','Saving a different post will clear the previous ranking. Continue?'))){changing=true;if(el.post.value!==pendingReelId)el.post.value='';el.change.classList.add('hidden');message(t('پۆستی نوێ هەڵبژێرە و پاشەکەوتی بکە.','Select the new post and save.'))}});
  el.selectReel.addEventListener('click',async()=>{
    el.selectReel.disabled=true;
    el.reelNote.textContent=t('لە پشکنینە...','Verifying Reel and Page...');
    try {
      const reel=await request('/api/meta/reel?url='+encodeURIComponent(el.reelUrl.value.trim()));
      if(!assets.some(a=>a.id===reel.pageId&&a.platform==='facebook'))throw new Error(t('پەیجی Reel ـەکە لەم پەیوەندییەدا نییە.','The Reel Page is not connected here.'));
      if(el.asset.value!==reel.pageId){el.asset.value=reel.pageId;await loadPosts(reel.pageId)}
      if(!Array.from(el.post.children).some(option=>option.value===reel.id))el.post.add(new Option(reel.message||`Facebook Reel ${reel.id}`,reel.id));
      el.post.value=reel.id;pendingReelId=reel.id;renderContext();
      el.reelNote.textContent=giveaway?.exists&&giveaway.postId!==reel.id&&!changing?t('Reel پشتڕاست کرایەوە و هەڵبژێردرا. بۆ جێگرتنەوەی پۆستی ئێستا، «گۆڕینی پۆست و دەستپێکردنەوە» پشتڕاست بکەرەوە، پاشان Save بکە. هێشتا هیچ داتایەک نەگۆڕاوە.','Reel verified and selected. To replace the current post, confirm Change post and reset, then Save. No data has changed yet.'):t('Reel پشتڕاست کرایەوە و وەک پۆست هەڵبژێردرا. بۆ بەکارهێنانی Save بکە.','Reel verified and selected as the post. Save to use it.');
    } catch(e) {el.reelNote.textContent=e.message}
    finally {el.selectReel.disabled=false}
  });
